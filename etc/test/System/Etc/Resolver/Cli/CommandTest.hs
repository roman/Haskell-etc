...
with_command_option_tests = testGroup
  ...
  , testCase "entry gets validated with a type" $ do
    ...
    case
        resolveCommandCliPure spec "program" ["test", "--greeting", "hello cli"]
      of
...
with_command_argument_tests = testGroup
  ...
  , testCase "supports same cli input on multiple arguments" $ do
    ...
    (cmd1, config1)           <- resolveCommandCliPure spec
                                                       "program"
                                                       ["test", "-g", "hello"]
    (cmd2, config2) <- resolveCommandCliPure spec
                                             "program"
                                             ["other", "-g", "hello"]
...
with_command = testGroup
  "when command given"
  [with_command_option_tests, with_command_argument_tests]
...
without_command = testCase "fails when command not given" $ do
  ...
  case resolveCommandCliPure spec "program" [] of
    Left err -> case fromException err of
      ...
      _ ->
        assertFailure ("Expecting sub-command to be required; got " <> show err)
